...
---
```
сборка: docker build -t gopherledger .
запуск: docker run  -p {желаемый порт}:8080 (опционально) -v {путь к конфигу}:/config.yaml -v {путь к файлу статистики}:/stats.txt gopherledger
```
в конфиге не требуется указание порта и хоста, это делается при запуске контейнера
---
### Критерии оценки
